...
model2 = glm(Y ~destination+passanger+weather+time+coupon+expiration+gender+maritalStatus+education+occupation+income+Bar+CoffeeHouse+RestaurantLessThan20+Restaurant20To50+toCoupon_GEQ25min+direction_same, data = trainingData, family = "binomial")
summary(model2)
...
model3 = glm(Y ~destination+passanger+weather+time+coupon+expiration+gender+maritalStatus+education+occupation+income+Bar+CoffeeHouse+Restaurant20To50+toCoupon_GEQ25min+direction_same,  family = "binomial")
summary(model3)


response<- ifelse(predict(model3, type = "response")>.5, 1, 0)  # predict distance
cor(response, trainingData$Y)
